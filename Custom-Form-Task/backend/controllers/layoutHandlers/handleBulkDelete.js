...
module.exports = async (identifier, body, t) => {
  ...
  for (const fieldRow of targetedFields) {
    ...
    if (fieldRow.options) {
      if (typeof fieldRow.options === 'string') {
        try { 
          currentOpts = JSON.parse(fieldRow.options); 
        } catch (e) { 
          currentOpts = {}; 
        }
  ...
  if (processedKeys.length > 0 && clientWorkspaceId) {
    for (let i = 0; i < processedKeys.length; i++) {
      const keyToDrop = processedKeys[i];
      await Form.update(
        { custom_values: sequelize.literal(`custom_values - '${keyToDrop}'`) },
        { where: { client_id: clientWorkspaceId }, transaction: t }
      );
    }
  }

  return { 
    message: `The entire ${target_type.toLowerCase()} layer '${target_type === 'SECTION' ? 
      section_name : area_name}' was successfully deactivated, soft-deleted fields archived, and custom records purged.` 
  };
};
