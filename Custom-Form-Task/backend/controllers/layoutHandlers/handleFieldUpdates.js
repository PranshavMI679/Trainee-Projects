...
module.exports = async (identifier, reqBody, t) => {
  ...
  if (!fields || !Array.isArray(fields)) {
    ...
    fields = [{ 
      key: reqBody.key, label: reqBody.label, type: reqBody.type, 
      is_required: reqBody.is_required, length: reqBody.length, options: reqBody.options 
    }];
  }
  ...
  for (let i = 0; i < fields.length; i++) {
    ...
    if (field.is_delete === true || field.is_delete === 'true') {
      ...
      if (existingLayout.options) {
        currentOptions = typeof existingLayout.options === 'string' ? JSON.parse(existingLayout.options) : existingLayout.options;
      }
      ...
      await DeleteHistory.create({
        ...
        archived_options: existingLayout.options || {}, action_type: 'SINGLE_FIELD_DELETE'
      }, { transaction: t });
    ...
    let processedOptions = existingLayout.options && typeof existingLayout.options === 'string' ? JSON.parse(existingLayout.options) : (existingLayout.options || {});
